...
def menuScreen():
    ...
    title = o.Text("algxbra", font_size=title_size) 
    play_sub_title = o.Text("play", font_size=sub_title_size)
    options_sub_title = o.Text("options", font_size=sub_title_size)
    quit_sub_title = o.Text("quit", font_size=sub_title_size)
    ...
    while main_menu_loop:
        ...
        if play_rect.collidepoint(pygame.mouse.get_pos()):
            ...
            if Click:
                play_cnt_dwn_screen = o.Tk_gui("Get ready...")
                back_btn = play_cnt_dwn_screen.btn("Back", play_cnt_dwn_screen.root.destroy)
                btn_info = (back_btn,)
                size = play_cnt_dwn_screen.packing_btns(1, btn_info)
                play_cnt_dwn_screen.timer(size)
                play_cnt_dwn_screen.main_loop()
        else:
        ...
        if quit_rect.collidepoint(pygame.mouse.get_pos()):
            ...
            if Click:
                quit_screen = o.Tk_gui("Are you sure you want to quit?")
                back_btn = quit_screen.btn("Back", quit_screen.root.destroy)
                quit_btn = quit_screen.btn("Quit", quit_screen.destroy_root_quit_game)
                btn_info = (back_btn, quit_btn)
                quit_screen.packing_btns(2, btn_info)
                quit_screen.main_loop()
        else:
...
def play_confir():
    ...
    w = Label(root2, text="Get ready...", font=(m.title_font, 25), bg="white")
    w.pack(pady=20, side=TOP)

    # Buttons
    back_button = Button(root2, text="Back", command=(lambda: play_back()), bg="black", fg="white", font=(m.title_font, 15))
    back_button.pack(pady=50, padx=50, side=BOTTOM)

    # Timer functionality
    t = 5
    start = time.time()
    t_label = Label(root2, text=count_down(t), font=(m.title_font, 33), bg="white", fg="red")
